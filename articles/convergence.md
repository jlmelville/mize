...
Stopping and point selection answer different questions. A tolerance,
limit, or failure decides when a run ends. When objective values are
available for best-result tracking, `mize` returns the observed
optimizer state with the lowest objective, even if the stopping event
occurred at a later state.
...
When objective tracking is active, use `par` or `best_par` for the
lowest observed objective. Use `last_par` when you need the parameters
associated with the terminal optimizer state. If no objective value
establishes best-result tracking, `mize` may instead compare observed
infinity norms of the gradient.

The following constructed run makes the distinction visible. Its
constant step moves from a lower objective to a stationary point with a
higher objective. The gradient tolerance stops the run at the stationary
point, but `mize` returns the lower-objective starting point.
...
| Point         | Parameter | Objective | Gradient infinity norm |
|:--------------|----------:|----------:|-----------------------:|
| Returned best |         0 |         0 |                      1 |
| Terminal      |         1 |         1 |                      0 |

Here `terminate$val = 0` is the gradient norm that satisfied `ginf_tol`
at `last_par = 1`. The returned `ginfn = 1` is evaluated at `par = 0`.
These values describe different points, so they do not conflict.

Cached function or gradient information can make the best and last
results differ even when every tolerance is disabled. A hard callback
budget can also prevent a final observation. In that case, an objective
result field can be absent, and `last_f` can be `NA` when `last_par`
differs from `par`. Check that the required field is present and
non-missing before using it programmatically.
