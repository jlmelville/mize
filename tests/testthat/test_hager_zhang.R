...
test_that("Hager-Zhang scale rescue is budget-aware and callback-free", {
  calls <- 0L
  evaluate <- function(alpha, calc_gradient = FALSE) {
    calls <<- calls + 1L
    list(alpha = alpha, value = NA_real_)
  }
  initial_point <- list(alpha = 0, value = 1, slope = -1)

  rescued <- propose_next_hager_zhang_alpha(
    evaluate,
    previous_alpha = 1,
    initial_point = initial_point,
    scale_estimate = 1e-9,
  ...
  expect_equal(rescued$unrescued_alpha, 2)
  expect_equal(rescued$alpha, 1e-9)
  expect_equal(rescued$required_contractions, 31)
  expect_equal(rescued$available_contractions, 18)
  expect_true(rescued$rescue_applied)

  calls <- 0L
  affordable <- propose_next_hager_zhang_alpha(
    evaluate,
    previous_alpha = 1,
    initial_point = initial_point,
    scale_estimate = 1,
    trial_evaluation_budget = 2,
    scale_rescue = TRUE
  )
  unbounded <- propose_next_hager_zhang_alpha(
    evaluate,
    previous_alpha = 1,
    initial_point = initial_point,
    scale_estimate = 1e-9,
  ...
  default_off <- propose_next_hager_zhang_alpha(
    evaluate,
    previous_alpha = 1,
    initial_point = initial_point,
    scale_estimate = 1e-9,
  ...
  expect_equal(affordable$alpha, 2)
  expect_false(affordable$rescue_applied)
  expect_equal(unbounded$alpha, 2)
  expect_false(unbounded$rescue_applied)
  expect_equal(default_off$alpha, 2)
  expect_false(default_off$rescue_applied)
})
