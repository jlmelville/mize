...
do this. See the vignette on 'Metric Multi-Dimensional Scaling' for the sort of
scenario where it could make a difference.
...
The absolute tolerance criterion simply compares the values of consecutive 
values: if that gets small, then we stop optimizing. However, relative tolerance
is often preferred, because it measures the change in value relative to the size
of the values themselves. 

```{r Relative tolerance}
res <- mize(rb0, rb_fg, rel_tol = 1e-3)
...
Other ways to control the amount of time spent include setting the maximum
...
Normally, your use of `max_iter` will be to increase it beyond 100 for 
optimizations that need longer.
...
methods may do a lot more work within an iteration than others.
...
number of function and gradient evaluations is the usual determinant of how
long an optimization takes. You can therefore decide to terminate based
...
Finally, there are also gradient tolerances:
...
`grad_tol` uses the 2-norm (sometimes referred to as the Euclidean norm) of the 
gradient to measure convergence. This seems like a good stopping criterion
because it is always zero at a minimum, even if the function isn't. It is also
used to compare different methods in Nocedal and Wright's book. However, it
has also been recognized that it is not always reliable, see for instance this
...
calculated at the end of the iteration for this purpose can nearly 
always be re-used for the gradient descent calculation at the beginning of the
next iteration, whereas the function-based convergence requires the function
to be calculated at the end of the iteration and this is not always reused,
although for many line search methods it is.

By default, because most methods *do* make use of function values, and it 
provides the most reliable convergence criteria, `rel_tol` and `abs_tol` are 
non-`NULL` by default. If you really don't want convergence based on function
evaluation, they can be set to `NULL`. If you set all the criteria to NULL,
the only thing that will terminate the optimization is `max_iter`:

```{r max_iter as termination criterion only}
res <- mize(rb0, rb_fg, method = "L-BFGS", rel_tol = NULL, abs_tol = NULL, 
            step_tol = NULL)
# got to all 100 iterations
res$iter
# although with standard line search if the direction vector gets too small
# it won't do any line search after the 57ish iterations
res$nf
res$ng
```
...
Because this might be more output than you want, you can also set the 
...
`rel_tol`, this means that the function needs to have been evaluated
at the current value of `par`. A lot of optimization methods do this as part
of their normal working, so it doesn't cost very much to do the convergence
check. However, not all optimization methods do. If you specify a non-`NULL`
value for `rel_tol` and `abs_tol` and the function value isn't available, it
will be calculated. This could, for some methods, add a lot of overhead. 

There is a trade-off to be made here: although calculating the function value
just for convergence checking increases the run time of the optimization, it 
may be less costly than running the optimization for several un-necessary steps
when you'd otherwise have stopped. Also, the only way for `mize` to keep track
of the best set of parameters yet seen is by looking at the function value at
the end of each iteration. If the optimization method doesn't calculate the
function for each value of `par` and `check_conv_every` doesn't run every
iteration, then you will only get back the "best" value of `par` for which
a function evaluation was carried out. Given a combination of particularly 
heinous functions to optimize and unwise choices of optimization method and
configuration, you could end up with the optimizer diverging and producing
progressively worse results, at least temporarily. Convergence checking is
important under these conditions, to at least get back some sort of improved
set of parameters.

But if convergence checking at every iteration is too much of a burden, you can
...
overhead, so this is a non-issue. For methods where it might be an issue, this
will be mentioned when we discuss that method.
...
iteration number as input and returns a scalar, which will be used as the 
momentum value, although the value will be clamped between 0 and 1.

For example, you could create a momentum schedule that randomly picks a value
between 0 and 1:
```{r momentum with random momentum}
mom_fn <- function(iter) {
...
resn <- mize(rb0, rb_fg, max_iter = 100, method = "MOM", mom_schedule = 0.9, 
             mom_type = "nesterov", store_progress = TRUE)
...
hand, you can see that the cost function is higher after 100 iterations with
Nesterov momentum. On yet another hand, `mize` returns the best value found,
not the last value found, and that happens to be better with nesterov momentum.
...
While the `"nesterov"` momentum type is available, I recommend using the 
`"NAG"` method instead, which calculates the gradient at the expected location
and is therefore less likely to interact with other parts of `mize` 
(like adaptive restart, see below) in unexpected ways. You can use a different
`mom_schedule` with the NAG just like with the momentum method.

## Line Searches

All the above methods need a line search to find out how far to move in the
gradient descent (i.e. non-momentum) part. The default for all methods mentioned
so far is a Wolfe line search that satisfies the strong Wolfe conditions. 
Specifically, [More-Thuente line search](https://doi.org/10.1145/192115.192132),
converted from 
...
If the More-Thuente line search fails to work for some reason, an alternative
...
Default values use the advice given in the Nocedal and Wright book: the
`c1` parameter is set to `1e-4`, while `c2` is set to `0.9` for quasi-Newton
methods (BFGS and L-BFGS) and `0.1` for everything else. For a line search
involving the strong Wolfe conditions, the `c2` parameter measures how strict
the line search is: the smaller it is (it cannot be set smaller than `c1` or
greater than 1), the tighter the line search. This is an important property
for some conjugate gradient methods to maintain their convergence properties.
...
one involving a quadratic interpolation. By default, the quadratic 
interpolation method is tried. 
...
point apart from the gradient, so all the things I've seen use a step size
that is effectively inversely proportional to the length of the gradient:

* Carl Edward Rasmussen's 
[conjugate gradient routine](http://learning.eng.cam.ac.uk/carl/code/minimize/)
uses $\frac{1}{1+\left|| g \right||_2^2}$ (the square of the 2-norm of the 
gradient)
* [SciPy](https://www.scipy.org/)'s `optimize.py` uses
$\frac{1}{\left|| g \right||_2}$ (the 2-norm of the gradient)
* Mark Schmidt's 
[minimization routines](http://www.cs.ubc.ca/~schmidtm/Software/minFunc.html)
uses $\frac{1}{1+\left|| g \right||_1}$ (the 1-norm of the gradient)

By default I have arbitrarily plumped for the Rasmussen (presumably it doesn't
make a huge difference), but you can explicitly choose by passing one of 
`"rasmussen"`, `"scipy"` or `"schmidt"` to the `step0` argument, e.g.:
...
starting from `step0`, until the sufficient decrease condition of the Wolfe 
line search conditions is fulfilled (controlled by `c1`, the larger the value,
the larger the decrease in the function value is needed for a step size to be
accepted). The step size is reduced by a factor of `step_down` until an 
acceptable step size is found - which can be very small.
...
the gradient restart (blue line) looks more effective.
