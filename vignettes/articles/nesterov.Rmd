...
*December 25 2025*: Here's a possible insight into the meaning of the gradient
term. If we write the steepest descent update as $s_t = - \varepsilon_t \nabla f\left(\theta_t \right)$, then the NAG update is:
...
\right]
$$
The last term is then the change in the steepest descent step. In the case where
gradient directions are not changing much between iterations,
$s_{t} - s_{t-1} \approx 0$, and the NAG update reduces to classical momentum.
But consider a case where the gradient is changing a lot between iterations,
for example, a steep ravine where we are bouncing from one side to the other. In
that case the direction (sign) of the gradient could be reversing in each
iteration, i.e. $s_{t} \approx - s_{t-1}$. In that case, the last term becomes
approximately $2 \mu_t s_t$, which increases the contribution of the current
steepest descent direction to the overall update and therefore reduces the
weight we place on previous directions. This gives some intuition about the
...
[notes on QHM](https://jlmelville.github.io/mize/articles/qhm.html) I wrote for my own
benefit.
...
\end{equation}
$$
This is exactly the same form as the classical momentum update except with
$b_t$ instead of $m_t$ or $v_t$. You can therefore define both forms of
momentum in a similar way with an identical recursive momentum buffer
definition.
...
So a very similar recursive form for the momentum buffer for both CM and NAG,
but using momentum from a different time step, and then a different velocity
vector for the update. The Dozat paper uses a slightly different indexing
convention for the momentum coefficients (shifted by one time step), but that's
...
very similar form. You can get something close to QHM by considering a
generalization of Dozat Nesterov momentum, replacing $\mu_{t-1}$ in the
definition of $m^{\prime}_{t+1}$ with a new free parameter, $\nu_{t}$. To avoid
any confusion let's call this new update $q_{t+1}$:
...
all too accustomed to. The reason this is only a family resemblance to QHM is
that QHM takes care to both separate the *direction* of the update vector versus
its magnitude. This takes us further away from how NAG and classical momentum
are often defined, however.
...
Finally, as an extra check, the NAG gradient descent result is shown in the final
