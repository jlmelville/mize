...
here. If you look at the Sutskever paper, t's important to note that the
...
$\theta_1$, not $\theta_0$. Sorry if you find that annoying.
...
\theta_4 & = \theta_3 + s_3 + \mu v_3 \\
 & = \theta_3 + s_3 + \mu \left(s_2 + \mu s_1 \right) \\
 & = \theta_3 + s_3 + \mu s_2 + \mu^2 s_1 \\
\\
\theta_5 & = \theta_4 + s_4 + \mu v_4 \\
 & = \theta_4 + s_4 + \mu \left( s_3 + \mu s_2 + \mu^2 s_1 \right)\\
 & = \theta_4 + s_4 + \mu s_3 + \mu^2 s_2 + \mu^3 s_1 \\
\end{split}
\end{equation}
$$
...
$v_t = 0$ and $s_{t-1} = 0$ and leave us with:

$$
\theta_{2} = \theta_1 + \left(1 + \mu \right) s_1
$$
...
versions of NAG a bit harder than it needs. Plus, I see the appeal of both
classical momentum and NAG taking the step on the first iteration when there is
no history of updates to work on. So I won't blame you if you decide to go with
steepest descent with step size of length $\varepsilon_1$ rather than 
...
\begin{equation}
\begin{split}
\theta_2 & = \theta_1 + \left(1 + \mu \right) s_1 \\
\\
...
\\
\theta_5 & = \theta_4 + \left(1 + \mu \right) s_4 - \mu s_3 + \mu v_4 \\
& = \theta_4 + \left(1 + \mu \right) s_4 - \mu s_3 + \mu \left[ \left(1 + \mu \right) s_3 + \mu^2 s_2 + \mu^3 s_1 \right] \\
& = \theta_4 + \left(1 + \mu \right) s_4 - \mu s_3 + \mu \left(1 + \mu \right) s_3 + \mu^3 s_2  + \mu^4 s_1 \\
& = \theta_4 + \left(1 + \mu \right) s_4 + \mu^2 s_3 + \mu^3 s_2 + \mu^4 s_1
\end{split}
\end{equation}
$$
...
\\
\theta_3 & = \theta_2 + \left(1 + \mu \right) s_2 - \mu s_1 + \mu v_2 \\
 & = \theta_2 + \left(1 + \mu \right) s_2 - \mu s_1 + \mu s_1 \\
 & = \theta_2 + \left(1 + \mu \right) s_2 \\
\\ 
\theta_4 & = \theta_3 + \left(1 + \mu \right) s_3 - \mu s_2 + \mu v_3 \\
& = \theta_3 + \left(1 + \mu \right) s_3 - \mu s_2 + \mu \left(1 + \mu \right) s_2 \\
& = \theta_3 + \left(1 + \mu \right) s_3 + \mu^2 s_2 \\
\\
\theta_5 & = \theta_4 + \left(1 + \mu \right) s_4 - \mu s_3 + \mu v_4 \\
& = \theta_4 + \left(1 + \mu \right) s_4 - \mu s_3 + \mu \left[ \left(1 + \mu \right) s_3 + \mu^2 s_2 \right] \\
& = \theta_4 + \left(1 + \mu \right) s_4 - \mu s_3 + \mu \left(1 + \mu \right) s_3 + \mu^3 s_2 \\
& = \theta_4 + \left(1 + \mu \right) s_4 + \mu^2 s_3 + \mu^3 s_2
\end{split}
\end{equation}
$$
...
The regularity of the difference between classical momentum and NAG weights in
that terrible suggest that there should be a way to express the NAG update in
...
*16 December 2021*: A new section (currently in a serviceable but somewhat
unsatisfactory state) on the expression for Nesterov momentum given by Timothy
Dozat in the NAdam paper.

All the expressions for CM and NAG so far have been recursive in their
update vectors, i.e. $v_t$ is always defined in terms of $v_{t-1}$. QHM does
things slightly differently and always uses the classical momentum update as the
recursive definition, even when its free parameter is set to give the same
results as NAG or an interpolation between CM and NAG. So instead of storing
the previous update, to use QHM, you:

1. Calculate the update vector as if you were doing classical momentum, which
requires the previous iteration's classical momentum update vector to have been
stored. Let's call the CM update vector $m_{t+1}$, and the previous iterate's
momentum update is therefore $m_t$.
1. Use the current iteration CM update vector, $m_{t+1}$ to generate the QHM 
update vector, which we can call $v_{t+1}$.
1. Update $\theta_{t}$ using $v_{t+1}$ just as you do with CM and NAG.
1. Store the current iteration CM update vector so it can be used as $m_t$ in
the next iteration.
1. You *don't* store $v_{t+1}$ as the next iteration's $v_t$, because as the
above steps show, it's not used for calculating the next $v_{t+1}$.

This is also how Nesterov momentum is defined in a paper by 
[Dozat](https://openreview.net/forum?id=OM0jvwB8jIp57ZJjtNEZ) which
integrates Nesterov momentum into the [Adam](https://arxiv.org/abs/1412.6980)
stochastic gradient descent method. The procedure there is:

1. $m_{t+1} = \mu m_{t} + s_{t}$
2. $v_{t+1} = \mu m_{t+1} + s_{t}$

Note that step 2 is the same as CM except $m_t$ is replaced by $m_{t+1}$. This
basically says that for CM, you carry out the gradient/velocity re-weighting
once, but for NAG you repeat the process a second time. I am not sure what to
call this. I'd love to say "double momentum" but that's 
[already a thing](https://arxiv.org/abs/2102.07367). I'm going with "iterated"
momentum.

Seems neat, but is it actually true for the definition of NAG we've been using?

At this point I must come to you with my head hung in defeat. I really wanted to
get from the NAG equation we are now all very familiar with to the Dozat version
of Nesterov momentum in the same way I generated the Sutskever and Bengio
versions, but I have yet to succeed (in case you're wondering, no I haven't
spent the last five years trying and failing).

Instead, I just wrote out the Dozat momentum for the first few iterations and
compared it to the unrolled CM and NAG expressions we generated above. It works
well enough for our purposes here.

The CM expression for $\theta_5$ re-written to use $m_t$ is the only example we
need:
...
If we apply the re-weighting a second time we would get:

$$
\begin{equation}
\begin{split}
\theta_5 & = \theta_4 + \mu m_5 \\
 & = \theta_4 + s_4 + \mu m_4 \\
 & = \theta_4 + s_4 + \mu \left( s_4 + \mu s_3 + \mu^2 s_2 + \mu^3 s_1 \right) \\
 & = \theta_4 + \left( 1 + \mu \right) s_4 + \mu^2 s_3 + \mu^3 s_2 + \mu^4 s_1
\end{split}
\end{equation}
$$
Here's a reminder of the equivalent NAG expression:

$$
\theta_5 = \theta_4 + \left(1 + \mu \right) s_4 + \mu^2 s_3 + \mu^3 s_2 + \mu^4 s_1
$$

So it seems like we have another expression for NAG, at least for the constant
momentum case.
...
momentum-style update. To differentiate from the Sutskever and Bengio versions
