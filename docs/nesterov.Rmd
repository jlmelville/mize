...
Similarly to QHM, we can write a related "generalized" momentum update by 
introducing a parameter $\beta$ to give:
...
to the first gradient descent direction after the first iteration, so the
second step also be a pure gradient descent step, but with an extra big step
