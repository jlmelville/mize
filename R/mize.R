...
#'
#'
#' * `"SD"` is plain steepest descent. Not very effective on its own,
#' but can be combined with various momentum approaches.
#' * `"BFGS"` is the Broyden-Fletcher-Goldfarb-Shanno quasi-Newton
...
#' * `"SR1"` is the Symmetric Rank 1 quasi-Newton method, incorporating
...
#' funconstrain package <https://github.com/jlmelville/funconstrain>), it
...
#' directly and so can scale to larger-sized problems than `"BFGS"`. The
#' amount of memory used can be controlled with the `memory` parameter.
#' * `"CG"` is the conjugate gradient method. The `cg_update`
...
#'   * `"PR+"` The method of Polak and Ribiere with a restart to
...
#'   * `"HZ+"` The method of Hager and Zhang with restart, as used
#'     in CG_DESCENT.
#'   * `"PRFR"` The modified PR-FR method of Gilbert and Nocedal
#'     (1992).
#'   The `"PR+"` and `"HZ+"` are likely to be most robust in practice.
#'   Other updates are available more for curiosity purposes.
#' * `"TN"` is the Truncated Newton method, which approximately solves
...
#' * `"PHESS"` is a partial-Hessian Newton variant that reuses a
#' Hessian factorization. It is accepted by `make_mize`, but should be
...
#' * `"Momentum"` is steepest descent with momentum. See below for
#' momentum options.
#'
...
#' * `"Backtracking"` carries out a back tracking line search using
...
#'   `step_down` between 0 and 1 (e.g. step size will be halved if
#'   `step_down` is set to `0.5`). If a constant step size reduction
...
#'   descent method. For method `"SD"` only, setting the
#'   `norm_direction` parameter to `TRUE` will scale the direction
#'   vector so it has unit length.
#'
#' If using one of the methods: `"BFGS"`, `"L-BFGS"`, `"CG"` or
#' `"NAG"`, one of the Wolfe line searches: `"Rasmussen"` or
#' `"More-Thuente"`, `"Schmidt"` or `"Hager-Zhang"` should be
...
#'
#'
#' * `c1`: The sufficient decrease condition. Normally left at its
#'    default value of 1e-4.
#' * `c2`: The sufficient curvature condition. Defaults to 0.9 if
#'    using the methods `"BFGS"` and `"L-BFGS"`, and to 0.1 for
#'    every other method, more or less in line with the recommendations given
#'    by Nocedal and Wright. The smaller the value of `c2`, the stricter
#'    the line search, but it should not be set to smaller than `c1`.
#' * `step0`: Initial value for the line search on the first step.
#'    If a positive finite numeric value is passed as an argument, that value is
#'    used as-is. Otherwise, by passing a string as an argument, a guess is made
...
#'   * `"scipy"`: As used in `optimize.py` in the python
#'      library Scipy.
#'      \deqn{\frac{1}{\left|g\right|}}{1 / |g|}
#'   * `"schmidt"`: As used by Schmidt in `minFunc.m`
#'      (the reciprocal of the l1 norm of g)
#'      \deqn{\frac{1}{\left|g\right|_1}}{1 / |g|1}
#'   * `"hz"`: The method suggested by Hager and Zhang (2006) for
#'      the CG_DESCENT software.
#'
#'    These arguments can be abbreviated.
#' * `step_next_init`: How to initialize alpha value of subsequent
...
#'   * `"hz"`: The QuadStep method of Hager and Zhang (2006) for
#'      the CG_DESCENT software.
#'   * scalar numeric: Set to a numeric value (e.g.
#'      `step_next_init = 1`) to explicitly set alpha to this value
#'      initially.
#'
#'    These arguments can be abbreviated. Details on the first two methods
#'    are provided by Nocedal and Wright.
#' * `try_newton_step`: For quasi-Newton methods (e.g. `"TN"`,
#'    `"BFGS"` and `"L-BFGS"`), setting this to `TRUE` will try
#'    the "natural" step size of 1, whenever the `step_next_init` method
#'    suggests an initial step size larger than that.
#' * `strong_curvature`: If `TRUE`, then the strong curvature
#'    condition will be used to check termination in Wolfe line search methods.
#'    If `FALSE`, then the standard curvature condition will be used. The
...
#' `"Schmidt"` and `"More-Thuente"` default to using the strong
...
#'   a multiplication.
#'
#' The `"bold driver"` line search also uses the `step_up` and
#' `step_down` parameters with similar meanings to their use with the
#' `"DBD"` method: the backtracking portion reduces the step size by a
#' factor of `step_down`. Once a satisfactory step size has been found, the
...
#'   `"nesterov"` (case insensitive, can be abbreviated). Using
#'   `"nesterov"` applies the momentum step before the
...
#'   `mom_init`) to another (`mom_final`) at a
#'   a specified iteration (`mom_switch_iter`).
...
#' strategies:
#'
#' * `"fn"`: A restart is applied if the function does not decrease
#'   on consecutive iterations.
#' * `"gr"`: A restart is applied if the direction of the
#'   optimization is not a descent direction.
#' * `"speed"`: A restart is applied if the update vector is not
...
#' is `"nesterov"`, the gradient-based restart is not available. The
#' `restart_wait` parameter controls how many iterations to wait after a
...
#'   software.
#' * `ginf_tol`: Absolute tolerance of the infinity norm (maximum
#'   absolute component) of the gradient. Indicated by `terminate$what`
#'   being `"ginf_tol"`.
#' * `step_tol`: Absolute tolerance of the step size, i.e. the
#'   Euclidean distance between values of `par` fell below the specified
#'   value. Indicated by `terminate$what` being `"step_tol"`.
...
#' @param norm_direction If `TRUE`, then the steepest descent direction
...
#' @param scale_hess if `TRUE`, the approximation to the inverse Hessian
...
#' line step value of 1 as the initial step size whenever `step_next_init`
#' suggests a step size > 1. Defaults to `TRUE` for quasi-Newton methods
#' such as BFGS and L-BFGS, `FALSE` otherwise.
...
#' results to the console. Ignored if `verbose` is `FALSE`.
#' If not an integer multiple of `check_conv_every`, it will be set to
#' `check_conv_every`.
...
#' * `ginfn`: Optional: the infinity norm (maximum absolute component)
#'  of the gradient vector, evaluated at the returned value of `par`.
...
#' *SIAM Journal on optimization*, *2*(1), 21-42.
#'
...
#' *Computational Optimization and Applications*, *22*(1), 5-35.
#'
...
#' In *Advances in Neural Information Processing Systems* (pp. 2510-2518).
#'
#' Sutskever, I. (2013).
...
#' *ACM Transactions on Mathematical Software (TOMS)*, *25*(1), 108-122.
#'
